...
class ExaminationPlan(models.Model):
    """Модель для плана обследования пациента"""
    ...
    name = models.CharField(
        "Название плана обследования",
        max_length=200,
        help_text="Например: 'Первичное обследование', 'Контрольное обследование'"
    )
    description = models.TextField(
        "Описание плана обследования",
        blank=True
    )
    priority = models.CharField(
        "Приоритет",
...
class ExaminationLabTest(models.Model):
    """Модель для лабораторных исследований в плане обследования"""
    ...
    def __str__(self):
        return f"{self.lab_test_assignment.lab_test.name} - {self.examination_plan.name}"
    
    def get_lab_test_name(self):
        """Возвращает название лабораторного исследования"""
        return self.lab_test_assignment.lab_test.name


class ExaminationInstrumental(models.Model):
    """Модель для инструментальных исследований в плане обследования"""
    ...
    instrumental_procedure = models.ForeignKey(
        'instrumental_procedures.InstrumentalProcedureDefinition',
        on_delete=models.CASCADE,
        null=True,
        blank=True,
        verbose_name="Инструментальное исследование из справочника",
        related_name="examination_instrumental_procedures"
    )
    custom_procedure = models.CharField(
        "Собственное исследование",
        max_length=200,
        blank=True,
        help_text="Введите название инструментального исследования в свободной форме"
    )
    instructions = models.TextField(
        "Особые указания",
        blank=True,
    ...
    def __str__(self):
        procedure_name = self.instrumental_procedure.name if self.instrumental_procedure else self.custom_procedure
        return f"{procedure_name}"
    
    def clean(self):
        """Проверяем, что либо выбрано исследование из справочника, либо введено собственное"""
        if not self.instrumental_procedure and not self.custom_procedure:
            raise ValidationError("Необходимо выбрать инструментальное исследование из справочника или ввести собственное")
        
        if self.instrumental_procedure and self.custom_procedure:
            raise ValidationError("Нельзя одновременно выбирать исследование из справочника и вводить собственное")
    
    def get_procedure_name(self):
        """Возвращает название инструментального исследования"""
        if self.instrumental_procedure:
            return self.instrumental_procedure.name
        elif self.custom_procedure:
    ...
    class OptimizedManager(models.Manager):
        ...
        def get_queryset(self):
            return super().get_queryset().select_related(
                ...
                'examination_plans__lab_tests__lab_test',
                'examination_plans__instrumental_procedures__instrumental_procedure',
