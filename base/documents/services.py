...
class DocumentPrintService:
    ...
    def __init__(self):
        ...
        # Настройки шрифтов
        self.font_name = "times"  # Times New Roman по умолчанию
        self.font_bold_name = "times-b"  # Times New Roman Bold по умолчанию
    ...
    def get_available_fonts():
        ...
        # Встроенные шрифты FPDF2
        fonts.append({
            'name': 'Times',
            'display_name': 'Times New Roman (встроенный)',
            'type': 'system'
        })
        fonts.append({
            'name': 'Times-B',
            'display_name': 'Times New Roman Bold (встроенный)',
            'type': 'system'
        })
        fonts.append({
            'name': 'Helvetica',
            'display_name': 'Helvetica (встроенный)',
            'type': 'system'
        })
        fonts.append({
            'name': 'Helvetica-B',
            'display_name': 'Helvetica Bold (встроенный)',
            'type': 'system'
        })
        ...
        if os.path.exists(fonts_dir):
            for font_file in os.listdir(fonts_dir):
                if font_file.lower().endswith(('.ttf', '.otf')):
                    ...
                    fonts.append({
                        'name': font_name,
                        'display_name': f'{font_name} (пользовательский)',
                        'type': 'custom',
    ...
    def set_font(self, font_name):
        ...
        # Если это пользовательский шрифт, проверяем его наличие
        if font_name not in ['Times', 'Times-B', 'Helvetica', 'Helvetica-B']:
            fonts_dir = os.path.join(settings.BASE_DIR, 'documents', 'templates', 'documents', 'fonts')
            ...
            if os.path.exists(font_path):
                ...
                print(f"Пользовательский шрифт {font_name} не найден, используем Times New Roman")
                self.font_name = "times"
                self.font_bold_name = "times-b"
        else:
            # Системные шрифты
            self.font_name = font_name.lower()
            if font_name == "Times":
                self.font_bold_name = "times-b"
            elif font_name == "Times-B":
                self.font_bold_name = "times-b"
            elif font_name == "Helvetica":
                self.font_bold_name = "helvetica-b"
            elif font_name == "Helvetica-B":
                self.font_bold_name = "helvetica-b"
            print(f"Установлен системный шрифт: {font_name}")
    
    def generate_pdf(self, clinical_document, template_name=None):
        """
        Генерирует PDF документ для печати с помощью FPDF2 для надежной поддержки кириллицы.
        """
        if FPDF2_AVAILABLE:
            return self._generate_pdf_with_fpdf2(clinical_document, template_name)
        else:
    ...
    def _generate_pdf_with_fpdf2(self, clinical_document, template_name=None):
        """
        ...
        try:
            ...
            # --- ГЛАВНОЕ ИЗМЕНЕНИЕ: Добавляем скачанный шрифт ---
            fonts_dir = os.path.join(settings.BASE_DIR, 'documents', 'templates', 'documents', 'fonts')
            font_path = os.path.join(fonts_dir, 'DejaVuSans.ttf')
            
            if not os.path.exists(font_path):
                print("!!! ШРИФТ DejaVuSans.ttf НЕ НАЙДЕН !!!")
                print("Используем встроенный шрифт Times New Roman")
                # Используем встроенный Times New Roman вместо DejaVu
                ...
                print("✅ PDF успешно сгенерирован с помощью FPDF2 и встроенного Times New Roman.")
                return pdf_bytes

            # Добавляем шрифт в FPDF под псевдонимом 'DejaVu'
            pdf.add_font('DejaVu', '', font_path, uni=True)
            pdf.add_font('DejaVu', 'B', font_path, uni=True) # Добавляем жирную версию (используя тот же файл)
            
            # --- Используем этот шрифт для всего документа ---
            ...
            for field_name, field_value in document_data.items():
                if field_value:
                    pdf.set_font('DejaVu', 'B', 12) # Метка поля - жирным
                    field_label = self._get_field_label(clinical_document.document_type.schema, field_name)
                    pdf.cell(0, 8, f"{field_label}:", ln=True)
                    
                    pdf.set_font('DejaVu', '', 12) # Значение поля - обычным
                    formatted_value = self._format_field_value(field_value)
                    pdf.multi_cell(0, 8, formatted_value)
                    pdf.ln(5)
            ...
            # Подпись
            y_before_footer = pdf.get_y()
            if y_before_footer > 250: # Если контент ушел вниз, добавляем новую страницу
                 pdf.add_page()
                 
            pdf.set_y(-30) # Позиционируем подвал в 3 см от низа страницы
            pdf.set_font('DejaVu', '', 10)
            pdf.line(10, pdf.get_y(), 200, pdf.get_y()) # Разделительная линия
            pdf.ln(5)
            ...
            print("✅ PDF успешно сгенерирован с помощью FPDF2 и шрифта DejaVu.")
            return pdf_bytes
